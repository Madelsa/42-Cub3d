...
/*   Updated: 2024/05/21 14:28:49 by aalkaisi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//     // Get the address of the buffer and retrieve image data format information
//     unsigned int *image_data = (unsigned int *)mlx_get_data_addr(buffer, &bits_per_pixel, &size_line, &endian);
//     unsigned int *texture_data = (unsigned int *)mlx_get_data_addr(map->textures[3], &bits_per_pixel, &size_line, &endian);
//     for (int i = 0; i < map->no_of_rays; i++) {
//         float perpDistance = map->player->ray[i]->distance * cos(map->player->ray[i]->ray_angle - map->player->rotation_angle);
//         float distanceProjPlane = ((WINDOW_WIDTH) / 2) / tan(FOV / 2);
//         float projectedWallHeight = (TWO_D_TILE_SIZE / perpDistance) * distanceProjPlane;
...
//             if (i >= 0 && i < WINDOW_WIDTH && y >= 0 && y < WINDOW_HEIGHT) {
//                 if (map->player->ray[i]->is_north_wall == 1 || map->player->ray[i]->is_south_wall == 1)
//                 {
//                     int ratio = wallStripHeight / 720;
//                     if (wallStripHeight > WINDOW_HEIGHT)
//                     {
//                         int dup = y / ratio;
//                         int top_texture_pixel = 720 * ((ratio - 1) / 2) / ratio;
//                         // int top_pixel = wallTopPixel /ratio;
//                         // printf("%f ", map->player->ray[i]->wall_hit_y);
//                         if (dup * WINDOW_HEIGHT + top_texture_pixel * WINDOW_HEIGHT + (int)(map->player->ray[i]->wall_hit_x * 10) >= 720*720)
//                         {
//                             printf("%d, %d, %d\n", dup, top_texture_pixel, (int)map->player->ray[i]->wall_hit_x);
//                             printf("%d ", dup * WINDOW_HEIGHT + top_texture_pixel * WINDOW_HEIGHT + (int)(map->player->ray[i]->wall_hit_x * 11.25));
//                             exit(1);
//                         }
//                         image_data[y * WINDOW_WIDTH + i] = texture_data[(dup * WINDOW_HEIGHT + top_texture_pixel * WINDOW_HEIGHT + (int)(map->player->ray[i]->wall_hit_x * 11.25))];
//                     }
