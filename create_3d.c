...
/*   Updated: 2024/05/21 18:26:59 by mabdelsa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	determine_texture_offset(t_map *map, int i)
{
	...
	if (map->player->ray[i]->was_hit_vertical)
		map->map_3d->tex_x = (int)map->map_3d->hit_y % TWO_D_TILE_SIZE;
	else
		map->map_3d->tex_x = (int)map->map_3d->hit_x % TWO_D_TILE_SIZE;
...
void	render_wall_slice(t_map *map, int i)
{
	...
	{
		d = y * 256 - WINDOW_HEIGHT * 128
			+ (int)map->map_3d->proj_wall_height * 128;
		map->map_3d->tex_y = ((d * 720) / (int)map->map_3d->proj_wall_height)
			/ 256;
		if (map->map_3d->tex_y < 0)
			map->map_3d->tex_y = 0;
		if (map->map_3d->tex_y >= 720)
			map->map_3d->tex_y = 720 - 1;
		if (i >= 0 && i < WINDOW_WIDTH && y >= 0 && y < WINDOW_HEIGHT)
		{
			map->map_3d->image_data[y * WINDOW_WIDTH
				+ i] = map->map_3d->tex_datas[map->map_3d->tex_index]
			[map->map_3d->tex_y * 720 + map->map_3d->tex_x];
		}
		y++;
	}
}
